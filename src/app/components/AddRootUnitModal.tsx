...
function CreateRootUnitForm({ subjectTitle, subjectId, setModalOpen } : CreateRootUnitPanelProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/admin/units/create-root', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          subjectId,
      ...
      if (!response.ok) {
        ...
        throw new Error(error.details || error.error || 'Failed to create unit');
      }
...
function CreateRootUnitPanel({subjectTitle, subjectId, setModalOpen}: CreateRootUnitPanelProps) {
  return  <DialogPanel
              transition
            ...
            >
              ...
              <CreateRootUnitForm subjectTitle={subjectTitle} subjectId={subjectId} setModalOpen={setModalOpen} />
            </DialogPanel>
}
...
export default function AddRootUnitModal(props: AddRootUnitModalProps) {
  ...
  const {subjectTitle, subjectId} = props;
  const panel = <CreateRootUnitPanel subjectTitle={subjectTitle} subjectId={subjectId} setModalOpen={setModalOpen} />
  return (
    <>
    <Button onClick={() => setModalOpen(!modalOpen)}>Create New Unit</Button>
