...
export interface NumberOfChecksData {
  date: string,
  count: number,
}

interface NumberOfChecksChartProps {
  data: NumberOfChecksData[];
  type: 'line' | 'area' | 'bar';
}

export const NumberOfChecksChart: React.FC<NumberOfChecksChartProps> = ({ data, type }) => {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-background p-3 border border-gray-200 dark:border-gray-800 rounded-lg shadow-lg">
          ...
          <p className="text-blue-600 dark:text-blue-400">{`Check Count: ${data.count}`}</p>
        </div>
      );
    }
    return null;
