...
function EditUnitForm({ unit, unitPath, updateUnit, setModalOpen, onSuccess }: EditUnitFormProps) {
  const [images, setImages] = useState<UploadedImage[]>(
    unit.imageUrls?.map(url => ({ url, key: url })) || []
  );
  ...
  const removeImage = async (key: string) => {
    try {
      ...
      if (!unit.imageUrls?.includes(key)) {
