...
function generateS3Key(localPath: string): string {
  ...
  // Preserve the directory structure from the local path
  const relativePath = localPath.replace(process.cwd() + '/public/images/', '');
  return `units/${relativePath}`;
}
...
async function migrateImagesToS3(): Promise<MigrationResult> {
  ...
  try {
    ...
    for (const unit of units) {
      ...
      for (const imageUrl of unit.imageUrls) {
        result.totalImages++;
...
async function main() {
  console.log('🚀 Starting unit image migration to S3...\n');
  ...
  console.log(`Total Units scanned: ${result.totalUnits}`);
